...
public class AttackTest {
	...
	public void testInvalidDefenderRoll() {
		int[] result = null;
		try {
			result = gamer1.getStrategy().rollDiceAttacker(indi, 3);
		...
		try {
			result = gamer1.getStrategy().rollDiceAttacker(indi, 4);
	...
	public void testInvalidAttackerRoll() {
		int[] result = null;
		try {
			result = gamer1.getStrategy().rollDiceDefender(indi, 2);
		...
		try {
			result = gamer1.getStrategy().rollDiceDefender(indi, 3);
	...
	public void testValidRoll() {
		int[] result = null;
		try {
			result = gamer1.getStrategy().rollDiceAttacker(vn, 3);
		...
		int[] result1 = null;
		try {
			result1 = gamer1.getStrategy().rollDiceDefender(vn, 2);
			assertNotNull(result);
		}catch(IllegalArgumentException e) {
			fail();
		}
	...
	public void testGoToBattle() {
		...
		int[] result = gamer1.getStrategy().goToBattle(attackerDice, defenderDice);
		assertEquals(2, result[0]);
		assertEquals(0, result[1]);
		
		int[] attackerDice1 = {6, 3, 1};
		int[] defenderDice1 = {5, 2};//defender loses both rounds
		int []result1 = gamer1.getStrategy().goToBattle(attackerDice1, defenderDice1);
		assertEquals(0, result1[0]);
		assertEquals(2, result1[1]);
		
		int[] attackerDice2 = {6, 3, 1};
		int[] defenderDice2 = {5, 4};//each loses 1 round
		int []result2 = gamer1.getStrategy().goToBattle(attackerDice2, defenderDice2);
		assertEquals(1, result2[0]);
		assertEquals(1, result2[1]);
		
		int[] attackerDice3 = {6};
		int[] defenderDice3 = {5,1};//defender loses 1 round
		int []result3 = gamer1.getStrategy().goToBattle(attackerDice3, defenderDice3);
	...
	public void testInvade() throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		int[] attackerDice = {6, 3, 1};
		int[] defenderDice = {6, 4};//defender wins both rounds
		int[] result = gamer2.getStrategy().goToBattle(attackerDice, defenderDice);
...
//		reflectMethodCall.invoke(gamer2, result, usa, vn, 3);
		...
		gamer2.getStrategy().invade(result, usa, vn, 3);
...
//		reflectMethodCall.invoke(gamer2, result, usa, indi, 3);
		...
		gamer2.getStrategy().invade(result, usa, indi, 3);
	...
	public void testAllOutMode() {
		...
		gamer2.getStrategy().goAllOut(usa, indi);
...
//		reflectMethodCall.invoke(gamer2, result, usa, germany, 3);
		...
		gamer2.getStrategy().invade(result, usa, germany, 3);
